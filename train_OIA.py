...
def main(args):
    ...
    params_to_optimize = [
        ...
        {"params": [p for p in model.channel_adjust.parameters() if p.requires_grad]},
        {"params": [p for p in model.embedding_adjust.parameters() if p.requires_grad & args.adjust_channel != 768]},
        {"params": [p for p in model.transformer.parameters() if p.requires_grad]},
        {"params": [p for p in model.action_classifier.parameters() if p.requires_grad]},
        {"params": [p for p in model.reason_classifier.parameters() if p.requires_grad]},
    ...
    for epoch in range(epochs):
        ...
        # total_loss, lr = 0, 0
        val_res = evaluate(model, dataloader_val, label_embedding, device)
